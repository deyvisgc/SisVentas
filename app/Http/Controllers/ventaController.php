...
class ventaController extends Controller
{
   ...
   $query=DB::table('producto')
       ...
       ->take(5)->get();
*/
   $query=DB::select("SELECT p.idproducto,p.nombre_pro,p.Precio_Pro
            from producto as p WHERE( p.nombre_pro LIKE '%".$produ."%' Or p.codigo LIKE '%".$produ."%')");
        foreach ($query as $quer)
        {
            $resulta[] = [ 'precio' => $quer->Precio_Pro, 'value' =>$quer->nombre_pro];
        }
    ...
    public function cargarClie(){
        $produ=Input::get('texto');
        $resulta=array();
        $query=DB::select("SELECT p.nombre ,p.Apellido_pat,p.Apellido_Materno,p.idtipoPersona,c.idcliente FROM cliente as c , tipopersona as p WHERE c.tipoPersona_idtipoPersona=p.idtipoPersona and p.nombre
LIKE '%".$produ."%' OR p.Apellido_pat LIKE '%".$produ."%' OR p.Apellido_Materno LIKE '%".$produ."%'");
        foreach ($query as $quer)
        {
